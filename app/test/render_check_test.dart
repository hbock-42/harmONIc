...
///
/// These write a PNG each, when asked. Read them.
///
/// The assertions run every time; the pictures are opt-in, and the results are
/// not committed — a golden is a picture of one machine's font rendering, and
/// text comes out as boxes here because the test font has no glyphs. That is
/// fine for what they are for: layout, colour and geometry. Wording is what
/// the assertions are for, and see [_draw] for why the two were separated.
///
///   fvm flutter test --dart-define=draw=true --update-goldens \
///     test/render_check_test.dart

/// Writes the picture, when somebody has asked for pictures.
///
/// The assertions in this file — that a badge is there, that nothing
/// overflowed, that the variants row appeared — are about layout and wording
/// and mean the same on every machine. Only the *comparison of pixels* is
/// machine-specific, because a golden is a photograph of one computer's font
/// rendering.
///
/// Those two used to be skipped together behind a tag, and the assertions
/// quietly rotted: twice, a check in here was wrong for several commits
/// because nothing ran it. So the assertions run with everything else now, and
/// only this last step waits to be asked:
///
///   fvm flutter test --dart-define=draw=true --update-goldens \
///     test/render_check_test.dart
///
/// then look at what lands in test/goldens.
Future<void> _draw(Finder what, String path) async {
  if (!_drawing) return;
  await expectLater(what, matchesGoldenFile(path));
}

const bool _drawing = bool.fromEnvironment('draw');
...
void main() {
  testWidgets('the canvas, with wires that have to go round things', (
    ...
    await _draw(find.byType(GraphCanvas), 'goldens/canvas.png');
  });
  ...
  testWidgets('the editor, with a buried card and a banner over the canvas', (
    ...
    await _draw(find.byType(EditorScreen), 'goldens/editor_banner.png');
  });
  ...
  testWidgets('a real build somebody sent in, fitted to the window', (
    ...
    await _draw(find.byType(GraphCanvas), 'goldens/real_build.png');
  });
  ...
  testWidgets('and close up, where the wires actually bend', (tester) async {
    ...
    await _draw(find.byType(GraphCanvas), 'goldens/close_up.png');
  });
  ...
  testWidgets('the marks a card wears: OVER, NEEDS, SPARE', (tester) async {
    ...
    final pipeline =
        (PipelineBuilder(testDatabase, name: 'over-drawn')
    ...
    await _draw(find.byType(GraphCanvas), 'goldens/marks.png');
  });
  ...
  testWidgets('the inspector on a Glo Squid, where milking can be declined', (
    ...
    await _draw(find.byType(InspectorPanel), 'goldens/inspector_squid.png');
  });
  ...
  testWidgets('the inspector on a Hatch nobody grooms, which now says so', (
    ...
    await tester.pumpAndSettle();
    expect(
      textContaining('eats and makes a fifth, lays a tenth'),
      findsOneWidget,
    );
    expect(tester.takeException(), isNull, reason: 'and it fits');

    await _draw(find.byType(InspectorPanel), 'goldens/inspector_ungroomed.png');
  });
  ...
  testWidgets('and all of it in the light theme, which nothing has checked', (
    ...
    await _draw(find.byType(EditorScreen), 'goldens/light_theme.png');
  });
  ...
  testWidgets('the geothermal boiler, as handed over in a share code', (
    ...
    final pipeline =
        (PipelineBuilder(testDatabase, name: 'Geothermal sour gas boiler')
    ...
    await _draw(find.byType(GraphCanvas), 'goldens/boiler.png');
  });

  testWidgets(
    'the catalogue, one row per thing rather than per way of keeping it',
    (tester) async {
      ...
      await _draw(find.byType(PalettePanel), 'goldens/palette.png');
    },
  );

  testWidgets(
    'mid-drag: the moving card loses its routes and nothing else does',
    (tester) async {
      ...
      await tester.pumpAndSettle();
      await _draw(find.byType(GraphCanvas), 'goldens/drag_before.png');
      ...
      expect(
        controller.pipeline.nodeOrThrow('h2out').y,
        greaterThan(pipeline.nodeOrThrow('h2out').y),
      );
      await _draw(find.byType(GraphCanvas), 'goldens/drag_during.png');

      controller.endNodeDrag();
      await tester.pumpAndSettle();
      await _draw(find.byType(GraphCanvas), 'goldens/drag_after.png');
    },
  );
  ...
  testWidgets('two ranches, one groomed and one not, side by side', (
    ...
    final pipeline =
        (PipelineBuilder(testDatabase, name: 'two ranches')
    ...
    await _draw(find.byType(GraphCanvas), 'goldens/two_ranches.png');
  });
  ...
  testWidgets('the first thing anybody sees: an empty app', (tester) async {
    ...
    await _draw(find.byType(EditorScreen), 'goldens/empty.png');
  });
}
