...
| E4-74 | P3 | The Critter Condo: the three buildings and a habitat for each critter | the mechanism it was blocked on landed in E10-95, so this is data now and no longer a rule. What is left: three Condo buildings (terrestrial, airborne, aquatic), a habitat recorded for each of the 39 critters -- which this app has never held -- and a condo input on each critter naming the one that suits it. Cannot be one shared building or one shared item, or a Pip would sleep in an aquatic Condo. The happiness arithmetic that made this hard is in test/happiness_test.dart, including the proof that no pair of factors could have held all four cases |
