...
| E4-74 | P3 | The Critter Condo, and why a factor cannot express it | +1 happiness where grooming gives +5. Reproduction is 1 + 2.25h, so h=0 lays at 100 per cent, a Condo at 325 and a groomed critter at 1225 -- and the two stack, a groomed critter in a Condo laying at 1450. That last figure is the whole difficulty. Happiness adds and port factors multiply, so no pair of them can hold all four cases: fixing the two single ones forces groomed-plus-Condo to 3981 per cent against the game's 1450. So withoutFactor cannot do it and neither can two of them. It wants a happiness quantity on the node, the way uptime and output scale already are, with the stations and fountains and Condos as what justifies the figure rather than what feeds it. Recorded here after saying twice that this only needed data, which was wrong. Three Condos as well -- terrestrial, airborne, aquatic -- and this app does not record which a critter is, though it does not record that for the two grooming stations either |
