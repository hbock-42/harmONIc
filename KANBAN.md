...
| E4-76 | ✅ | The two critters with no wild form | the Glo Squid and the Seaquine, both Aquatic, were the 2 of 39 with no wild twin -- found by rendering the inspector for a Glo Squid and expecting the row of other ways to keep it. Both wild forms are 60 cycles an egg against 6 tame, which is the tenth the other 37 use and is stated on both pages. Wild drops grooming and milking and what they buy: no squid ink, no ovolene, the wiki saying outright that only a tamed Seaquine can be milked. Nothing checked that a tame critter has a twin at all -- the rule was only ever written the other way round, every wild critter matching its tame one, which says nothing about a tame one with no twin -- so that test exists now and fails if a wild form is taken away |
...
| E4-74 | P3 | The Critter Condo: blocked on two figures the wiki does not give | the mechanism landed in E10-95 so this is data, and the data was audited against oxygennotincluded.wiki.gg on 2026-08-31. Three buildings confirmed: Critter Condo (200 kg raw mineral + 10 kg fibre), Airborne Critter Condo (200 kg plastic) and Aquatic Critter Condo (200 kg plastic), all 3x3, no power, no Duplicant time, each giving the Cozy buff of +1 happiness for a cycle. Every critter's locomotion is now known from the game tags on its own page -- Walker 17, Walker+Amphibious 5 (gildgo, plug_slug, slogo, pokeshell, oakshell), Flyer 6, Swimmer 9, Hoverer 2 (slickster, molten_slickster), which is all 39. Morph pages redirect to the parent and share its tags, checked rather than assumed for sage/smooth hatch, glossy drecko, the three pufts, shatter flox and cuddle pip. Two things stop it there. Eligibility does not follow locomotion: the Condo page says terrestrial critters "excluding Pokeshell species" and the Aquatic Condo page says Pokeshells and Pacu, so a Walker can want the aquatic one -- and nothing anywhere says which Condo a Hoverer uses, so both Slicksters are unanswerable. And no source gives how many critters one Condo keeps happy: the buff lasts a cycle and they "regularly relax in it", which is the one number a port needs and the difference between one Condo a stable and one a critter. Wants the game files or a stopwatch in-game, not the wiki . What a Condo is worth is now known and is not what E4-74 first said: one point takes a critter from -1 to 0, which buys no eggs at all and takes the metabolism from a fifth back to whole. Five times the coal and not one egg more, which is a stranger building than the 325 % first recorded here |
...
| E10-88 | ✅ | A new content pack no longer hides itself, and other things checked rather than assumed | adding bionic would have hidden all of it from every existing player: load() replaces the enabled set with the saved list, and a pack absent from that list because it did not exist yet is indistinguishable from one turned off. Settings record knownPacks now, with the four pre-existing packs assumed for older saves. Found by going back over the week rather than by anybody hitting it. Also added: every process is now placed and solved in a build of its own (470 of them, all fine), every port of every process must carry something, needs is validated where the database loads, the two reported fixtures that were only ever round-tripped now pin what the app says about them, and the new content is checked to be reachable in the palette rather than merely present in the data |
| E10-87 | ✅ | Every recipe can actually be used | nothing checked it. A spec could be placed, wired to everything it asks for, and still refuse to solve, and the only way to find out was to draw it -- which nobody does 470 times. Three checks: it solves, it comes out at the size asked for (a recipe solving at zero has solved nothing, since every rate is a multiple of a count), and every port of it carries something (a port that can only carry nothing is a port that is not there). All clean today. Plus load-time validation of the new needs field: it must name a port the recipe has, that port must be an input, and only an output may depend on anything -- a typo there would otherwise be silent, because an output needing a port that does not exist is simply never switched off |
| E10-113 | ✅ | The building roster, and the Wood Burner it found | third of the outward audits, after plants and critters, and it found a miss on the first pass: this app had a Wood Heater and no Wood Burner. Two buildings -- one burns wood for heat, the other for 300 W -- so a base run off Arbor Trees had no generator to draw. 1200 g/s of wood, 300 W, 170 g/s of CO2 at 110 C, 9 kDTU/s, 800 kg of metal ore, 2x2, base game. Most of the mass does not come back, which is what burning something for power looks like, so it joins the coal and petroleum generators in expectedImbalance. Roster scoped to the categories that hold recipes -- Refinement, Food, power generation -- since a tile or a ladder converts nothing: 37 modelled, 7 excluded, 4 missing. Diamond Press is blocked on radbolts, which this app does not model at all, and Molecular Forge on a chain of materials it has never heard of (niobium, tungsten, isosap, brackwax, graphite) -- between them they are why diamond and insulite are items nothing can make. Also checked and found innocent: wood and lumber are not a duplicate pair, wood is a class and lumber is a member of it |
| E10-112 | ✅ | The Jawbo, the one missing critter that was blocking something | the Smoker already asked for jawbo fillet and nothing could make one -- bog jelly again with a critter missing instead of a plant. Added groomed and wild from the single published sentence: "A Jawbo consumes 1 unit of Pacu, or 1,000 kcal Fish Fillet every cycle and excretes up to 60 kg/cycle of Rust." Only the fillet half is a flow, since a critter eating a critter is not something this app can draw, and 1000 kcal at 1000 kcal/kg is a kilogram. Tagged unverified and the reason is on the card: a kilogram in and 60 kg of rust out is sixty times the mass, which belongs to the Pacu rather than the fillet, and the mass audit cannot catch it because anything laying eggs is exempt from weighing. It is also the only source of rust here, which fired the pack audit -- rust is base game and the Rust Deoxidizer is base game, so it is listed as mined rather than made, beside abyssalite and gold amalgam which are there for the same reason. Five audits fired in all and every one wanted something true |
| E10-111 | ✅ | The critter roster, the other half of the outward audit | plants had one and critters did not, which is the gap the Stone Hatch lived in. Transcribed from the wiki species list: 37 modelled, 3 feeding modes that are not species (two Grubgrub diets and a grazing Beakon), 4 excluded as untameable, and 17 missing -- 3 species this app has never heard of and 14 morphs of species it does keep. Jawbo is the sharpest of them: the Smoker already asks for jawbo fillet and nothing can make one, so it is the same fault as bog jelly with the critter missing instead of the plant. Lumb is odd the other way -- the Blum Lumb is modelled and its base form is not. Also settles the Kelpole, left open when the Gassy Moo was fixed: the wiki lists it among the untameable, so its grooming port buys nothing because nobody can groom one at all, and the test pins that it books no Duplicant time and does not start glum. It stays because an Orehull ranch runs on kelp and kelpoles and cannot be drawn without it |
| E10-110 | ✅ | Three of the seven missing plants, the ones with published figures | Grubfruit Plant (10 kg/cycle sulfur, 8/32 cycles, "8 kg or 2000 kcal" a harvest -- and grubfruit is 250 kcal/kg, so the wiki states the same fact twice and the two agree), Sweatcorn Stalk (10 kg/cycle of peat or coal, 3/12, 800 kcal which is a kilogram) and Plume Squash Plant (15 kg/cycle of ethanol, the only plant here watered with it, 9/36, 4000 kcal which is a kilogram). Crop mass is calories over calories-per-kilogram, which is the convention the Spindly Grubfruit Plant already used. Plume squash also had no kcalPerKg at all, so no eating node was generated and a Duplicant could not eat one. Four audits fired on the way in and each wanted something real: the roster, the small-rate list, the alternatives allowlist for peat-or-coal, and the wild-plant rule that a wild card must name the input it no longer takes. Four remain and each is blocked on a figure nobody publishes: Seakomb and Dew Dripper have no mass per unit, Ovagro yield depends on how many vines a node carries, and Mimika publishes nothing at all |
| E10-109 | ✅ | An audit that looks outward, since the inward ones cannot see what is absent | asked after Swamp Chard turned out to be missing too. Every audit here looks inward -- an item nothing makes, a port nothing needs, a rate too small -- and none can see a thing absent from both sides of the ledger, which is how the Stone Hatch was gone for the life of the project. The plant roster is transcribed from the wiki now and every name is modelled, excluded with a reason, or admitted missing: 23 modelled, 26 excluded, 7 missing. Swamp Chard turns out to be an exclusion rather than a gap -- the wiki says outright it is "incapable of propagating", so there is no pipeline to draw, the same reason Mussel Sprout was left out in E4-37 -- and the point is that the reason is now written where a test reads it rather than in a KANBAN sentence. The 7 missing all block recipes this app already has: Grubfruit, Plume Squash, Sweatcorn, Ovagro, Mimika, Dew Dripper, Seakomb. Hand-written and it will go stale when a pack ships, which is the price of the only check that can find something nobody thought of |
| E10-108 | ✅ | A palette test that stopped scrolling too early | adding two plants broke two folding tests, and neither was really about the plants. scrollTo returned as soon as a row existed in the tree, which for a lazy list means built rather than on screen -- so it could stop with its target at the very bottom edge, near enough to find and too far to tap, and the group it was meant to unfold stayed folded. Ends with ensureVisible now. The other test wanted both the SUPPLY and OUTPUT headers on one screen as evidence that the 428 rows between them are folded; two rows above was enough to part them, so it counts drags instead and allows one, which is the same claim without the arithmetic about screen height |
| E10-107 | ✅ | Bog jelly could not be made, and four other foods still cannot | reported as "I cannot do a bog jelly pipeline for feeding". Nothing produced bog jelly: the Bog Bucket was absent, while the Swampy Delights recipe asked for the jelly and the generated eating node stood ready to turn it into calories. Added watered and wild -- 40 kg/cycle of polluted water, 6.6 cycles for a harvest the wiki gives as 1840 kcal, and bog jelly is 1840 kcal/kg so the harvest is a kilogram exactly. The wiki prints growth as 6.6/26.4 without saying what the pair means; 26.4 is four times 6.6, which is how every other plant here has been read. The general fault is the point: an ingredient nothing makes is a recipe nobody can draw, and the app offers it anyway. There is an audit for it now and it found four more -- grubfruit, which blocks a preserve and all three Mixed Berry Pies, ovagro fig, sweatcorn, and jawbo fillet, which nothing cooks with either. It has to look at written recipes only, since every item gets a generated supply node and an audit built on the whole database would answer yes forever. Removing the Bog Bucket fails it |
| E10-106 | ✅ | The Stone Hatch was never in the database | asked "where is stone hatch?" and it was nowhere: hatch, sage and smooth were all here and the fourth morph was not. Nothing caught it, and the reason is worth keeping -- the roster test names six critters by hand and the morph test names three, so both were as complete as whoever wrote them remembered to be, and neither counts anything. Added tame and wild: on rock it is numerically a Hatch, 140 kg/cycle in and 70 kg of coal out, 6 cycles an egg groomed and 60 wild, 2 kg of meat over a 100-cycle life, walker. The metal ore it eats and a Hatch will not is described rather than modelled -- a critter eats by the calorie so another food is another weight, and the Smooth Hatch eating 100 kg/cycle of ore where this eats 140 kg of rock is how far apart those can be. Four counts moved deliberately as a result: 40 families, 40 tame and 40 wild, 38 groomed layers, 23 walkers |
| E10-105 | ✅ | Text fields ignored the mouse | reported as "each text input I cannot double click or cmd + a to select all". Double-click and drag were genuinely broken and the cause is structural: EditableText draws a cursor and a selection and answers the keyboard, and listens to the mouse not at all -- Material wraps it in a TextSelectionGestureDetector for that, and OniField was built straight on EditableText precisely to keep Material out, so it got none of it. A plain GestureDetector(onTap: requestFocus) was the entire selection story. Now built with TextSelectionGestureDetectorBuilder and rendererIgnoresPointer, so the detector owns the pointer instead of fighting the field for it. Cmd+A was reported too and is not broken: it works in a bare field and inside the editor. Worth knowing why it looked plausible -- a widget test runs as Android unless told otherwise, where select-all is Ctrl+A, so my first probe "reproduced" it by pressing a key that means nothing on that platform. The guard presses Ctrl and says so |
| E10-104 | ✅ | The two Plant Pulverizer brackene recipes that were deferred | E10-85 left the Sleet Wheat and Pincha ones out because the wiki table gives their output as a per-plant yield where the Nosh Bean row gives a weight. Mass settles it: every row of that table whose figures are all weights balances exactly -- 2+18 into 20, 3+17 into 20, 25+75 into 100, slime 100 into 70 and 30 -- so 10 grains and 15 kg of water make 25 kg. The grain weighs a kilogram, from three directions: 18 cycles to grow and 18 grains a harvest against this database's 1 kg of grain a cycle, and the Frost Bun and Pepper Bread recipes independently spending 3 and 10 grains a loaf. Same 40 s batch assumed as the sibling recipes, same UNVERIFIED tag, since the cycle time is still unpublished (E4-11). Also all three water ports now accept mucin, which all three descriptions have claimed since they were written while the port offered only water -- and the allowlist of ports making that claim wanted three new lines, which is the guard working |
| E10-103 | ✅ | Measured whether exact arithmetic is worth it, and it is not | asked whether package:decimal would remove floating-point error on big pipelines. Measured the error first: worst relative residual across the four shipped templates, a 500-deep pump chain, a 200-wide fan and a genuine petroleum feedback loop is 1.5e-16, under one unit in the last place of a double, and most ports are exact to the bit. There is nothing there to remove. Measured the cost second, on dense elimination with rationals on BigInt, which is what decimal becomes the moment you divide -- Decimal cannot hold 1/3 at all: 115x slower at 20 unknowns, 272x at 40, 283x at 60, and the factor keeps climbing because the numbers themselves grow, 78 digits then 163 then 256, about four digits a row. A 500-node build would carry 2000-digit coefficients; a 14 ms solve becomes seconds, and it re-solves on every edit while a drag frame already costs 17-24 ms. Against that, the rates come off a wiki with 4 to 6 significant figures and a double carries 15 to 16, so the uncertainty in the inputs is ten orders of magnitude larger than the arithmetic error. Where float noise did reach a user -- Wilkeu's "-0.000000 g/cycle" -- it was a display fault and the fix was capping printed digits at four; the solver already compares against 1e-6 and 1e-9 tolerances rather than zero, which is the right shape for this |
| E10-102 | ✅ | The shipped changelog was never held to the one in docs | copy_docs.sh has always copied two files into the assets and only one of them, the guide, had a test saying the shipped copy matches. A stale changelog is quieter than a stale guide and worse: the what's-new notice reads the shipped copy, so somebody would be shown the release before last and never know. Also pins that no two releases share a heading, since the heading is an entry's identity -- it is what is stored as the last thing you read and what the unread count counts back to, so two alike and the count silently reports the distance to the first. Both fail when broken on purpose. Ran tool/smoke.sh as well, which nothing today had: the real bundle builds, the guide is in it, and the app opens and stays up |
| E10-101 | ✅ | Stop skipping the render checks, skip only the pixels | the golden tag skipped a whole file, and the file holds two different kinds of check. Whether a badge is present, whether anything overflowed, whether a row appeared -- layout and wording, the same on every machine -- went out with the pixel comparison, which is the only part that is a photograph of one computer's fonts. So twice an assertion in there was wrong for several commits and nothing ran it. The comparison is behind a dart-define now and everything else runs with the suite: 784 tests and one skip become 797 and none, and it is no slower, because comparing images was the expensive part. Breaking a render assertion now fails a plain flutter test, which is the check that this was worth doing |
| E10-100 | ✅ | Say what declining a line costs, and run the goldens that had gone stale | switching grooming off moves a build by five and the app said nothing about why. The line reads off the model -- metabolismAt and layingAt against the happiness with and without -- so a Hatch says a fifth and a tenth, a Moo only the fifth, and a Glo Squid's milking says nothing at all, since milking buys no happiness. Rendered and looked at, twice: the first picture was 900 tall and the Hatch's description pushed every port below the fold, so it could not show the thing it was taken for. Running the golden suite also turned up two stale checks the skip tag had been hiding for three commits -- an assertion that the Glo Squid has no wild twin, which E4-76 gave it, and the two-ranch picture, whose ungroomed half now reads a fifth. Tagged tests do not rot any slower for being skipped |
| E10-99 | ✅ | The Gassy Moo, whose grooming bought nothing | found by pinning that every tame critter starts at -1 happiness, which two did not. A Moo lays no eggs and the only thing happiness moved was eggs, so nothing on the card declared any and its twelve seconds a cycle of Duplicant time bought a number that never changed -- nor could the line be declined, since a port earns its switch by something depending on it. The metabolism column is what grooming buys a Moo: glum is a fifth, so 2 kg of natural gas a cycle against 10. The wiki says it from the other end -- a Moo's "reproduction is tied closer to its critter metabolism", so "a satisfied gassy moo is just as effective as a happy gassy moo", satisfied being zero happiness, one point off the cliff. Also fixes switchablePorts, which asked for an output priced in happiness as well and so left the Moo unswitchable even once its gas depended on it, and the wild Moo card, which carried the boilerplate about laying a tenth as often. The Kelpole is the other one and is left alone on purpose: stub page, empty ranching section, no Duplicant time, and giving it happiness would cut its nori to a fifth on a guess -- named in the test so it is a decision rather than an oversight |
| E10-98 | ✅ | The metabolism correction, checked on the screen rather than in the solver | E10-97 fixed the engine and nothing asked whether the number a person reads off a card moved with it. Twelve hatches with the grooming declined: a fifth of the coal, a fifth of the rock, a tenth of the eggs -- two columns that deliberately do not move together -- and the coal generator they were drawn to feed shrinks to a fifth with them, which is what somebody actually notices. Also that the card says OFF, since a build reading a fifth of what it did has to say why. Flattening the metabolism cliff fails two of the four and leaves the egg one alone, which is the check that they are measuring different things |
| E10-97 | ✅ | A tamed critter starts at -1, which moved every figure | the happiness mechanism shipped in E10-95 was right and its numbers were not. "Tamed critters have a base happiness of -1", quoted off the Critter page, so grooming's five points land at 4 and not 5. Two columns come off that number and only one was modelled. Reproduction is flat at 100 % anywhere below zero and 1 + 2.25h above it, so groomed is 1000 % and not 1225, ungroomed is a tenth rather than a twelfth, and a Condo alone -- -1 to 0 -- is worth no eggs whatever. Metabolism is the column that was missing entirely: "Glum, tame, critters have -80 % metabolism offset", so an ungroomed critter eats a fifth and makes a fifth, where this app asserted outright that metabolism does not care how happy a critter is. That was the larger error by far -- five times on every input and output of an unattended ranch -- and it had a test asserting it. Found by chasing whether milking really halves breeding, which it turns out no source says either |
| E10-96 | ✅ | The Seaquine milking that produced nothing | it had a milking input, so you could wire an Aquatic Milking Station to it and book the Duplicant, and no output needed it -- the port was not even switchable, since a port earns that by something needing it. Ovolene was in the catalogue made by nobody and used by nobody, which is the same fault seen from the other end, and it sat on the deliberately-unused list as "a rate nobody has published". Two pages publish it: 100 kg a milking. The one thing still unpublished is how often a critter can be milked, so this assumes once a cycle, which is what the Glo Squid beside it already assumed. Milking is also said to halve breeding, which needs a supplied input to scale an output down -- the mirror of needs/withoutFactor -- and is not modelled |
| E10-95 | ✅ | Happiness, so a Critter Condo can be said at all | ports carry happiness points on the way in and a happinessAt on the way out, and the solver turns the sum into a rate through 1 + 2.25h. Adding rather than multiplying is the whole point: grooming is 5 points and a Condo 1, and the four cases are 100, 100, 1000 and 1225 per cent once the -1 a tamed critter starts at is counted, which no pair of port factors can hold -- grooming multiplies the eggs by ten and a Condo alone by one, so as factors they would make 10 together and the game says 12.25. The 37 groomed critters moved across from needs/withoutFactor and give identical numbers, though the 0.08163 they carried turned out to be wrong as well and is 0.1; see E10-97, which corrected the arithmetic this row got right in shape and wrong in figures. Nothing a user can see yet: the Condo itself is data and is E4-74. Also broke each of the new data rules on purpose to check the complaint fires, and three older ones of the same family that had never been checked either -- assertConsistent had a dozen messages and one test, that the shipped data raises none of them, which says nothing about whether the checks work |
| E10-94 | ✅ | Measure what a big build costs to draw | the solver has been measured since E3-9 and the router since it was written; the app never had been, so every figure quoted about this project's speed was about code that runs once when the graph changes and none about the sixty times a second a drag asks for. A steady drag frame is about 17 ms at forty cards, 24 at a hundred and 110 at four hundred -- debug, widget test, raster left out, so some unknown multiple of a shipped build. Four times the cards costs 4.6 times the frame, so near enough linear and nothing quadratic hiding. The first version of this measurement pumped the canvas without a listener, measured 0.1 ms and would have reported the app as fast when it had drawn nothing; the test now asserts the card moved and that no card was culled. A repaint boundary per card takes 17 to 15.7 and 28.3 to 23.8, consistent across runs, and shifts two goldens by 0.01% of glyph-edge antialiasing. The cost is the rebuild rather than the paint, which is where a bigger win would have to come from |
| E10-93 | ✅ | Every shortcut pressed, on both keyboards | the suite pressed keys in nine files and every one of them held ⌘ -- on bindings that had already shipped as meta-only once, which is a key most keyboards do not have, so on Windows and Linux undo, redo, copy, paste and zoom did nothing. The fix asks the platform; nothing had ever pressed Ctrl to see whether it worked. Now each binding is pressed on both, and undo, redo and zoom out were pressed for the first time. It found one live bug: "⌘+" was bound to LogicalKeyboardKey.add alone, the plus of a keyboard where it needs no shift, so ⇧= and the number pad plus were both dead. Also pins that a nudge snaps to the grid rather than adding a cell, and notes why the arrow cases are a test each -- a second editor pumped into a live test loses its arrows to focus traversal |
| E10-92 | ✅ | A damaged save opens the app rather than stopping it | the store refuses anything that will not parse, so what gets through is JSON that parses and means something else -- a newer version, a half-flushed write, a hand edit. Each build was already parsed inside a try so one bad one costs you nothing else, but the envelope was cast straight: pipelines-not-a-list and lastOpenedId-not-a-string both threw out of load(), which runs at start-up, so the app simply would not open. Read rather than cast now. The openIds case passed at first for the wrong reason -- an empty build list makes the loader give up before reaching it -- so that test has a real build beside it and fails without the fix, as do the other two |
| E10-91 | ✅ | The first run, which nothing had ever checked end to end | the one screen every single person sees, and the state where a mistake about what a *missing* setting means does its damage -- there was one of those this week. Every store empty, every setting unset: it opens on an empty canvas with no exception, every pack including the newest is on, the catalogue can be searched from cold, and a build drawn on a first run is still there when the app is reopened. Rendered as well, and it is well composed: the explanation centred with its template and demo buttons, panels populated either side |
| E10-90 | ✅ | The editor fits the windows people actually have | never checked. This runs in a browser, so the window is whatever the reader has, and everything added this week -- a banner over the canvas, badges on cards, a row of other ways to keep a creature -- was drawn at a comfortable desktop size. Flutter reports a layout that will not fit as an exception, so it is askable: six sizes from 1280x800 down to 760x640 and a short 1280x560, all clean. The floor is about 560 px wide; 520 overflows by roughly a hundred in two places, which is narrower than any browser window anybody plans a base in, so it is recorded rather than fixed. Checked the assertion can fail by pointing it at a phone |
| E10-89 | ✅ | An unattended ranch: grooming declined, and the eggs that go with it | the rate half of the mechanism. Port.withoutFactor says what is left of an output when the input it needs is declined -- 0 for the ink, 100/1225 for the eggs, which is the published happiness arithmetic and is the same for all 37 groomed critters because it comes from the happiness and not the species. Metabolism is untouched: it eats and produces the same, and a test holds that so the ranch is not punished twice. Also fixes the labour, which _labourFor got wrong for declined grooming -- an unwired port means "somebody outside is doing it" and a declined one means nobody is. Not the Condo: that is a third state (325 %) rather than a second, and is still parked with its arithmetic |
| E10-89a | ✅ | A card says OFF when something on it is switched off | found by rendering two ranches side by side, one groomed and one not, and being unable to tell them apart -- a declined port looks exactly like one waiting for a supply, on a difference of twelve times the eggs. Amber, like VENT, which is the same kind of deliberate choice seen from the other side of the node. And a fourth panel pumped without a listener: the test that the mark goes away when the input comes back could not have passed, because the card never rebuilt |
...
| E10-72a | ✅ | And the walk reaches the week's new machinery, provably | it predated all of it: declining an input, keeping a creature another way, digging out a buried card, moving one anywhere. Four moves added, and then the thing this week keeps teaching -- a move that never fires is a move that is not tested, and a random walk is exactly the shape that goes quiet without saying so. Counted: 357 reveals, 178 swaps, 24 declines across 16 000 steps, and each now has a floor it must clear. Declines are rarest because only critters have a port that can be declined and the corpus is mostly buildings |
