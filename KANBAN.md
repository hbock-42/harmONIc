...
| E10-95 | ✅ | Happiness, so a Critter Condo can be said at all | ports carry happiness points on the way in and a happinessAt on the way out, and the solver turns the sum into a rate through 1 + 2.25h. Adding rather than multiplying is the whole point: grooming is 5 points and a Condo 1, and the four cases are 100, 325, 1225 and 1450 per cent, which no pair of port factors can hold -- fitting the two single cases forces the fourth to 3981. The 37 groomed critters moved across from needs/withoutFactor and give identical numbers, since the 0.08163 they carried is 1/12.25 exactly, which is the same rule written as one of its points. Nothing a user can see yet: the Condo itself is data and is E4-74. Also broke each of the new data rules on purpose to check the complaint fires, and three older ones of the same family that had never been checked either -- assertConsistent had a dozen messages and one test, that the shipped data raises none of them, which says nothing about whether the checks work |
