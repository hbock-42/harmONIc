...
| E10-47 | P1 | "The rest goes here" has no way to be said | an output node hung on a port that also feeds consumers is either a loose end (consumer-driven: nothing says how much) or a thief (producer-driven with no share: it takes the lot). Three reports are this one gap. Wants a third edge mode meaning "whatever is left after everything else on this port" -- which the solver could carry, since it is the balance term it already computes for an unwired port |
